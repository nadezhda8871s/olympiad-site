...
app.post('/api/upload-background', upload.single('background'), (req, res) => {
  ...
  settings.backgrounds[docType] = req.file.filename;
  fs.writeJsonSync(SETTINGS_FILE, settings);

  res.json({ success: true, message: `Фон для ${docType} загружен` });
});
...
app.post('/api/generate-pdf', async (req, res) => {
  ...
  const settings = fs.readJsonSync(SETTINGS_FILE);
  const bgPath = settings.backgrounds[template] || settings.backgrounds.all;
  const bgFullPath = bgPath ? path.join(__dirname, 'uploads', bgPath) : null;
  ...
  const background = bgPath && fs.existsSync(bgFullPath)
    ? `<img src="file://${bgFullPath}" style="position:absolute; top:0; left:0; width:100%; height:100%; z-index:0; opacity:0.1;">`
    : '';
  ...
  try {
    ...
    db.push({ id: Date.now(), timestamp: new Date().toISOString(), template, data });
...
app.get('/api/export-participants', (req, res) => {
  ...
  if (participants.length === 0) return res.status(400).json({ error: 'Нет данных' });
